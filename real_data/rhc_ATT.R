...
bootamw<-function(A,X,Y,M,bn=100,est="ATE"){
...
for (c in 2:34) {
  d=c-1
  difference[2*d-1]=abs(round((mean(zp[loc.1, c])-mean(zp[loc.0, c]))/sd(zp[, c]),3))
  difference[2*d]=abs(round((sum(zp[loc.1, c]*k[loc.1])/sample_size-
